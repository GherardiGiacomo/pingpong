...
/*** TO BE DONE START ***/
	...
	fd_set fdset;
	FD_ZERO(&fdset);
	FD_SET(ping_socket,&fdset);

	if (select(ping_socket + 1, &fdset, NULL, NULL, &tm) == -1)
		fail("errore nella select");
...
/** TO BE DONE START ***/
    ...
    if (write(ask_socket,request,strlen(request)) < 0)
        fail("Impossibile mandare la richiesta");
