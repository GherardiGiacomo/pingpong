	...
	while (n_left > 0) {
		...
		if (n_written < 0) {
			...
			/*** TO BE DONE START ***/
				...
				if (errno == EAGAIN || errno == EWOULDBLOCK) // se write fallisce (valore negativo e l'errore è EAGAIN o EWOULDBLOCK), continuiamo; Siamo quindi in modalità non bloccante
					continue;
